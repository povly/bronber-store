...
Настройки **подставляются в существующую статическую вёрстку**, а не заменяют её:
view composer в `AppServiceProvider` отдаёт `SettingsResolver::header()/footer()` —
плоские структуры с готовыми href (двухтипные ссылки: страница сайта с автолокализацией
или кастомный URL; битые пропускаются с WARN). Паршлы `top-bar`, `header`, `footer`
рендерят их через `@if` со статикой прототипа как fallback — поиск, каталог-меню,
Alpine-компоненты и дизайн не затрагиваются. Логотип, поиск, действия (избранное/профиль/
корзина), переключатель языков, кнопка «Каталог», оплата и вход — остаются кодом
(функциональные элементы, не контент).

Сидер `SettingsSeeder` наполняет настройки контентом, эквивалентным статике прототипа
(custom-ссылки на фиксированные роуты, per-locale повторный запуск не затирает правки
админа — заполняет только пустые `value`).
