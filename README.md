...
Проект на Laravel 13 + MoonShine 4. Сейчас находится на стадии **pre-MVP**: фронтенд-витрина — прототип
на Blade + Alpine.js; контент ключевых страниц (главная, FAQ, доставка и оплата, возврат и обмен, программа лояльности, страница 404), блог со статьями (`/blog`, `/blog/{slug}` — сущности `articles` + блоки `article-*`), шапка/подвал и мобильные меню уже управляются из
MoonShine через flexible-layouts блоки (DB-страницы; настройки шапки/подвала/меню — с fallback на прототип), остальное — захардкоженные
данные прототипа. Доменные модели, реальная корзина и оформление заказа — в планах развития.
