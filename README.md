...
на Blade + Alpine.js; контент ключевых страниц (главная, FAQ) и шапка/подвал уже управляются из
MoonShine через flexible-layouts блоки (DB-страницы + fallback на прототип), остальное — захардкоженные
dанные прототипа. Доменные модели, реальная корзина и оформление заказа — в планах развития.
