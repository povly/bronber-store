...
- **Страница «Программа лояльности» из БД** (slug `loyalty`, catch-all): блоки `loyalty-hero`
  (заголовок, описание, кнопка-ссылка двух типов «страница/кастом» через `BuildsLinkFields`, отдельные
  изображения десктоп/мобайл), `loyalty-benefits` (список иконка+текст-EditorJS; пункт с заполненным
  заголовком становится «главным» — большая иконка, модификатор `--main`), `loyalty-how-works`
  (заголовок + шаги: иконка/заголовок/описание-EditorJS, рендер `RenderEditorJs`) и `loyalty-bottom`
  (левая колонка: строки примера «левый/правый текст + цвет» (`MoonShine\UI\Fields\Color` → inline
  `color:` вместо классового `--accent`) + баннер-подарок иконка/заголовок/текст; правая: заголовок +
  FAQ-аккордеон); демо-контент — `LoyaltyPageSeeder` (ru/en, идемпотентный, тексты из lang-файлов
...
- **Страница ошибки 404 из настроек** (settings, ключ `error-404`, ru-fallback в `SettingService`;
  ...
  вариант оформления `primary`/`white-border`, `x-btn`); пустое значение настройки → статический
  прототип (паттерн шапки/подвала); `Error404BlockLibrary` + регистрация в `SettingResource` (form
  match, AJAX-union, фильтр/метки); демо-контент — `SettingsSeeder` (ключ `error-404`), ссылки
  кнопок — кастомные с локальным префиксом (`/`, `/en/`); published-страница со slug `404` не
  создаётся (catch-all не должен отдавать `/404` c HTTP 200)
...
- **Рецепт конвертации прототип-страницы в блоки**: 1) класс блока в `Blocks/{Page}/` + регистрация
  в `PageBlockLibrary` (+ слот в `mediaSchemas()` при медиа-полях); 2) вью `blocks/{page}/{name}`
  (классы и css прототипа, иконки через `<x-img>` — MediaManagerPicker хранит пути относительно
  storage-диска, нормализация `/storage/`); 3) идемпотентный сидер fill-when-empty (тексты из
  lang-файлов прототипа); 4) фиксированный роут не заводить — catch-all подхватит; 5) тесты по
  образцу `FaqPageTest`/`DeliveryPageTest`; 6) прототипные вью и lang-фолбэк удалить
