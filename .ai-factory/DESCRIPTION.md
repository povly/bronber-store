...
- **Страница «Программа лояльности» из БД** (slug `loyalty`, catch-all): блоки `loyalty-hero`
  ...
  изображения десктоп/мобайл), `loyalty-benefits` (список иконка+текст; пункт с заполненным заголовком
  становится «главным» — большая иконка, модификатор `--main`), `loyalty-how-works` (заголовок + шаги:
  иконка/заголовок/описание) и `loyalty-bottom` (левая колонка: строки примера «левый/правый текст +
  цвет» (`MoonShine\UI\Fields\Color` → inline `color:` вместо классового `--accent`) + баннер-подарок
  иконка/заголовок/текст; правая: заголовок + FAQ-аккордеон); демо-контент — `LoyaltyPageSeeder`
  (ru/en, идемпотентный, тексты из lang-файлов прототипа, иконки выгружены в
