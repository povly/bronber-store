        ...
        <div class="container">
                ...
                <div class="profile-orders__sort" x-data="{ sortOpen: false }">
                    ...
                    <div class="profile-orders__sort-select" @click.away="sortOpen = false">
                        <button type="button" class="profile-orders__sort-trigger" @click="sortOpen = !sortOpen">
                            ...
                            <svg class="profile-orders__sort-chevron" :class="{ 'is-open': sortOpen }" width="12"
                                height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M4.99 6.05a.75.75 0 0 0 1.06 0l4.78-4.78a.75.75 0 0 0-1.06-1.06L5.52 4.46 1.28.21a.75.75 0 0 0-1.06 1.06L4.99 6.05Z"
                                    fill="#BFBFBF" />
                            </svg>
                        </button>
                        <div class="profile-orders__sort-dropdown" x-show="sortOpen" x-transition style="display:none;">
                            <button type="button" class="profile-orders__sort-option is-active"
            ...
            <div class="profile-orders__list">
                @foreach ($orders as $order)
                    <x-order-card :image="$order['image']" :status="$order['status']" :order-number="$order['number']" :date="$order['date']"
                        :total="$order['total']" href="/profile/orders/1" />
                @endforeach
            </div>
...
@else
    <section class="profile-orders">
        <div class="container">
            <h2 class="profile-orders__title profile-orders__title--simple section__title">
                {{ __('profile.recent_orders_title') }}</h2>
            <div class="profile-orders__list">
                @foreach ($orders as $order)
                    <x-order-card :image="$order['image']" :status="$order['status']" :order-number="$order['number']" :date="$order['date']"
                        :total="$order['total']" href="/profile/orders/1" />
